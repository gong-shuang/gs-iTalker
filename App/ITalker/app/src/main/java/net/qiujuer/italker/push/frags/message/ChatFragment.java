...
public abstract class ChatFragment<InitModel>
    ...
    // 注意这个toolbar 和 appbar 的区别，这个是下拉后，显示的大图
    ...
    EditText mContent;    // 内容按钮

    @BindView(R.id.btn_submit)
    View mSubmit;    // 发送按钮，或者图片按钮

    // 控制顶部面板与软键盘过度的Boss控件
    private AirPanel.Boss mPanelBoss;
    private PanelFragment mPanelFragment;  //底部面板实现
...
//                getActivity().finish();   // toobar的返回键
                Intent intent = new Intent();
                intent.putExtra(MessageActivity.KEY_RECEIVER_ID, mReceiverId);
                getActivity().setResult(MessageActivity.RESULT_OK, intent);
                getActivity().finish();   // toobar的返回键
            }
        });
    }
