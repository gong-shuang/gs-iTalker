...
import net.qiujuer.web.italker.push.bean.api.group.GroupMemberDeleteModel;
...
import net.qiujuer.web.italker.push.bean.card.GroupResponseCard;
...
public class GroupService extends BaseService {
    /**
     * 创建群，在创建群的同时，也创建了群的成员
     * 问题，（1）必须要设置群的头像
     * （2）通过群的名字判断这个群是否已经创建了
     *
     * @param model 基本参数
     * @return 群信息
     */
    ...
    /**
     * 删除群成员，不能删除自己，只能删除别人，如果像删除自己，就是退出
     * 删除成员，请求的人必须是管理员
     *
     * @param groupId  群Id，
     ...
     */
    @POST
    @Path("/{groupId}/delete/member")
    //http:.../api/group/0000-0000-0000-0000/delete/member
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseModel<List<GroupMemberCard>> deleteMember(@PathParam("groupId") String groupId, GroupMemberDeleteModel model) {
        if (Strings.isNullOrEmpty(groupId) || !GroupMemberDeleteModel.check(model))
            return ResponseModel.buildParameterError();
        ...
        //如果删除的人存在，且在oldMemberUserIds中，则删除，否则不删除
        ...
        // 进行删除操作
        Set<GroupMember> deleteMembers =  GroupFactory.delMembers(group, deleteUsers);
        if(deleteMembers==null)
            return ResponseModel.buildServiceError();


        // 转换
        List<GroupMemberCard> deleteCards = deleteMembers.stream()
                .map(GroupMemberCard::new)
                .collect(Collectors.toList());
...
//        PushFactory.pushJoinGroup(insertMembers);
 ...
 //       PushFactory.pushGroupMemberAdd(oldMembers, insertCards);
        ...
        return ResponseModel.buildOk(deleteCards);
    }


    /**
     * 暂时定为，只能被好友邀请添加到群了，自己是不能主动添加到群里的。暂不实现
     *
     * 申请加入一个群，
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseModel<GroupResponseCard> quit(@PathParam("groupId") String groupId) {
        // 拿到我的信息
        User self = getSelf();
        ...
        // 进行删除操作
        boolean ok =  GroupFactory.quitGroup(group.getId(), self.getId());
        if(ok==false)
            return ResponseModel.buildServiceError();

        //转换结果
        GroupResponseCard quitCard = new GroupResponseCard();
        Set<String> list= new HashSet<String>() ;
        list.add(self.getId());
        quitCard.setGroupId(groupId);
        quitCard.setMembers(list);
        ...
        return ResponseModel.buildOk(quitCard);
    }
