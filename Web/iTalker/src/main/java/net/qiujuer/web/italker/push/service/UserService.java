...
import net.qiujuer.web.italker.push.utils.RegularUtils;
...
public class UserService extends BaseService {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseModel<List<UserCard>> follow() {
        User self = getSelf();

        // 通过self找到关注我的那些人
        List<User> users = UserFactory.findById(self);
        // 转换为UserCard
        List<UserCard> userCards = users.stream()
                // map操作，相当于转置操作，User->UserCard
                .map(user -> new UserCard(user, true))
    ...
    public ResponseModel<List<UserCard>> search(@DefaultValue("") @PathParam("name") String name) {
        ...
        // 先查询数据
        //判断是否为电话号码
        if(RegularUtils.isMobile(name)){
            searchUsers = UserFactory.searchByPhone(name);
        }else {
            searchUsers = UserFactory.search(name);
        }
