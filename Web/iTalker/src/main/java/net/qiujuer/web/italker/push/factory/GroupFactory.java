...
public class GroupFactory {
    ...
    // 给群删除成员
    public static Set<GroupMember> delMembers(Group group, List<User> delUsers) {
        return Hib.query(session -> {

            Set<GroupMember> members = new HashSet<>();

            for (User user : delUsers) {
                GroupMember member = (GroupMember) session
                        .createQuery("from GroupMember where userId=:userId and groupId=:groupId")
                        .setParameter("userId", user.getId())
                ...
                result.setPermissionType(member.getPermissionType());
                result.setNotifyLevel(member.getNotifyLevel());
                result.setGroupId(member.getGroupId());
                result.setUserId(member.getUserId());
                result.setCreateAt(member.getCreateAt());
                result.setUpdateAt(member.getUpdateAt());
                result.setUser(member.getUser());
                result.setGroup(member.getGroup());


                members.add(result);

                int deletedEntities = session
                        .createQuery("delete from GroupMember where userId=:userId and groupId=:groupId")
                        .setParameter("userId", user.getId())
                        .setParameter("groupId", group.getId())
                        .executeUpdate();
