...
public class UserFactory {
     ...
     * @return List<User>
     */
    public static List<User> findById(User self) {
     ...
     * 搜索联系人的实现（通过手机号码）
     *
     * @param phone 查询的phone，不允许为空
     * @return 查询到的用户集合，如果name为空，则返回最近的用户
     */
    @SuppressWarnings("unchecked")
    public static List<User> searchByPhone(String phone) {
        if (Strings.isNullOrEmpty(phone))
            return null; // 保证不能为null的情况
        final String searchPhone = "%" + phone + "%"; // 模糊匹配

        return Hib.query(session -> {
            // 查询的条件：name忽略大小写，并且使用like（模糊）查询；
            // 头像和描述必须完善才能查询到
            return (List<User>) session.createQuery("from User where lower(phone) like :phone and portrait is not null and description is not null")
